...
class TripSummaryScreen extends ConsumerWidget {
  ...
  const TripSummaryScreen({Key? key, this.trip}) : super(key: key);
      ...
      return Scaffold(
        ...
        body: Center(
          child: Padding(
            ...
            child: Text(
              'No trip data is available right now. Please return to the dashboard and start a new trip.',
              textAlign: TextAlign.center,
          ...
          children: [
            Card(
              child: Padding(
                ...
                child: Column(
                  children: [
                    Text("Eco Score", style: theme.titleMedium),
                    const SizedBox(height: 8),
                    Text(
                      formatEcoScore(tripData.ecoScore),
                      style: theme.displayLarge?.copyWith(color: AppColors.primary),
                    ),
                  ],
                ),
              ),
            ...
            Row(
              children: [
                Expanded(child: _buildStatCard("Distance", "${tripData.distanceKm.toStringAsFixed(1)} km", theme)),
                const SizedBox(width: 16),
                Expanded(child: _buildStatCard("Cost", "RM ${tripData.fuelCostRm.toStringAsFixed(2)}", theme)),
              ],
            ),
            const SizedBox(height: 16),
            _buildStatCard("Carbon Saved", "${tripData.carbonSavedKg.toStringAsFixed(2)} kg CO₂", theme),
            const Spacer(),
            ElevatedButton(
              onPressed: () {
                ref.read(appNavigationProvider.notifier).navigateTo(AppScreen.dashboard);
  ...
  Widget _buildStatCard(String title, String value, TextTheme theme) {
    return Card(
      child: Padding(
