...
class DashboardScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final impactString = ledgerController.translateImpact(ledgerController.totalCarbonSaved);
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          Text("Carbon Ledger", style: theme.displayMedium),
          const SizedBox(height: 24),
              ...
              child: Row(
                ...
                children: [
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text("Petrol Points Balance", style: theme.bodyMedium),
                      const SizedBox(height: 8),
                      Text("${ledgerState.petrolPointsBalance} pts", style: theme.titleLarge),
                    ],
                  ),
                  const Icon(Icons.local_gas_station, color: AppColors.primary, size: 40),
                ],
              ),
            ),
          ),
          ...
          Card(
            ...
            child: Padding(
              ...
              child: Column(
                children: [
                  const Icon(Icons.park, color: AppColors.primary, size: 48),
                  const SizedBox(height: 16),
                  Text("Your Concrete Impact", style: theme.bodyMedium),
                  const SizedBox(height: 8),
                  Text(
                    impactString,
                    textAlign: TextAlign.center,
          ...
          // Chart Section
          Text("Carbon Saved (Last 3 Trips)", style: theme.titleLarge),
          const SizedBox(height: 24),
          SizedBox(
            height: 250,
            child: BarChart(
              BarChartData(
                ...
                barGroups: _buildChartGroups(ledgerState.trips),
              ),
            ),
          ),
        ],
  ...
  List<BarChartGroupData> _buildChartGroups(List<TripModel> trips) {
    // Take the 3 most recent trips for the chart and reverse them for chronological display
    final recentTrips = trips.take(3).toList().reversed.toList();
    
    return List.generate(recentTrips.length, (index) {
      return BarChartGroupData(
        x: index,
        barRods: [
          BarChartRodData(
            toY: recentTrips[index].carbonSavedKg,
